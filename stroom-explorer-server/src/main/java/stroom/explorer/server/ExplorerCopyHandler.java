...
@TaskHandlerBean(task = ExplorerCopyAction.class)
class ExplorerCopyHandler
        extends AbstractTaskHandler<ExplorerCopyAction, ExplorerNode> {
    private final DocumentServiceLocator documentServiceLocator;
    private final ExplorerTreeDao explorerTreeDao;

    @Inject
    ExplorerCopyHandler(final DocumentServiceLocator documentServiceLocator, final ExplorerTreeDao explorerTreeDao) {
    ...
    @Override
    public ExplorerNode exec(final ExplorerCopyAction action) {
        final DocumentService documentService = documentServiceLocator.locate(action.getDocument().getType());

        ExplorerNode explorerNode;

        try {
            final DocRef docRef = documentService.copyDocument(action.getDocument().getDocRef(), action.getFolder().getDocRef(), action.getName());

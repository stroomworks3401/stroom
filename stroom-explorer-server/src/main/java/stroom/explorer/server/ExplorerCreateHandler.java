...
@TaskHandlerBean(task = ExplorerCreateAction.class)
class ExplorerCreateHandler extends AbstractTaskHandler<ExplorerCreateAction, ExplorerNode> {
    private final DocumentServiceLocator documentServiceLocator;
    private final ExplorerTreeDao explorerTreeDao;

    @Inject
    ExplorerCreateHandler(final DocumentServiceLocator documentServiceLocator, final ExplorerTreeDao explorerTreeDao) {
    ...
    @Override
    public ExplorerNode exec(final ExplorerCreateAction action) {
        final DocumentService documentService = documentServiceLocator.locate(action.getType());

        ExplorerNode explorerNode;

        try {
            final DocRef docRef = documentService.createDocument(action.getFolder().getDocRef(), action.getName());

            // Add the explorer entry.
